...
\begin{itemize}
	\item In SIMO Systems only coding and diversity gains can be exploited
	...
	\begin{itemize}
		\item how to choose combining weights \begin{math}w_n\end{math}?
...
\end{align*}
\begin{itemize}
	\item Instantaneous SNR: \begin{math}y_t = \frac{\epsilon_s}{\sigma_n^2}\times |h|^2  \end{math}
\item Average SNR: \begin{math}\bar{y}_t = \mathcal{E}\{y_t\} = \frac{\epsilon_s}{\sigma_n^2}\end{math}
\end{itemize}
\paragraph{Bit and Symbol Error Rate}
\begin{itemize}
	\item The Bit and Symbol Error Rate of many modulation schemes can be expressed for given \begin{math} y_t\end{math} as:
\end{itemize}
\begin{align*}
	P_e(y_t) = aQ\bigl\{ \sqrt{by_t}\bigr\}
\end{align*}
where:
\begin{itemize}
	\item \begin{math} Q(x) = \frac{1}{\sqrt{2\pi}}\times\int_{x}^{\infty} e^{-\frac{t^2}{2}}~dt
	\end{math}
	\item \begin{math}P_e(y_t) \end{math} may be exact or approximation
	\item BPSK: exact with \begin{math} a = 1, b = 2\end{math}
	\item M-ary QAM: tight approximation with \begin{math} a = 4\bigl (1-\frac{1}{\sqrt{M}}\bigr ), b =  \frac{3}{M - 1}   \end{math}
\end{itemize}
...
\begin{itemize}
	\item Alternative representation of Q\;-\;function:
			...
			\end{align*}
			\begin{math}\rightarrow\end{math} Integral limits are fixed and don't depend on integration variables!
	\item Average error probability
		\begin{align*} 
		P_e = \mathcal{E}\bigl \{P_e(y_t)\bigr \} = \int_{0}^{\infty}aQ\bigr (\sqrt{bx}\bigl )p_{\gamma_t}(x)~dx
		\end{align*}
		\begin{itemize}
			\item Integral may be difficult to solve analytically
			\item Integral has infinite support \begin{math} \rightarrow \end{math} numerical evaluation difficult
		\end{itemize}
	\item Using alternate representation of Q-function we get:
		\begin{align*}
...
where:
		\begin{itemize}
				\item \begin{math}M_{\gamma_t}(s) = \int_{0}^{\infty}p_{\gamma_t}(x)e^{-sx}~dx \end{math} is the Laplace transform of \begin{math} p_{\gamma_t}\end{math}	
				\item \begin{math}M_{\gamma_t}(-s)\end{math} is the so called Moment Generation Function (MGF) of \begin{math}p_{\gamma_t}\end{math}
				\item Here, we will also refer to \begin{math}M_{\gamma_t}(s)\end{math} as MGF
				\item \begin{math}M_{\gamma_t}(s)\end{math} is sometimes easier to obtain than \begin{math}p_{\gamma_t}\end{math}
