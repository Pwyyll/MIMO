...
%			\begin{align*}
%			p_{\gamma_t}&=\frac{N_R}{\bar{\gamma}}\left[1-\mathrm{exp}\left(-\frac{x}{\bar{\gamma}}\right)\right]^{N_R-1}\mathrm{exp}\left(-\frac{x}{\bar{\gamma}}\right)\\
%				&\overset{\bar{\gamma} \rightarrow \infty}{=} \frac{N_R}{\bar{\gamma}}\left[1-\left(1-\frac{x}{\bar{\gamma}}+O\left(\bar{\gamma}^{-1}\right)\right]^{N_R-1}\left(1-\frac{x}{\bar{\gamma}}+O\left(\bar{\gamma}^{-1}\right)\right)\\
...
%			\left[\rightarrow P_e&=\frac{a}{\pi}\int\limits^{\frac{\pi}{2}}_0 M_{\gamma_t}\left(\frac{b}{2\sin^2(\theta)}\right)\mathrm{d}\theta\right]
%			&=\frac{a(2N_R)!}{b^{N_R}2^{N_R+1}N_R!}\frac{1}{\bar{\gamma}^{N_R}}+O\left(\bar{\gamma}^{-N_R}\right)
%			\end{align*}
%		\begin{itemize}
%			\item[] $\Longrightarrow \text{Diversity gain: } G_d = N_R$
...
%		\begin{align*}
%			P_{out}=\mathrm{Pr}\{\gamma_{\hat{n}} \leq \gamma_T \}=P_{\gamma_\hat{n}}(\gamma_T=\left[1-\mathrm{exp}\left(-\frac{\gamma_T}{\bar{\gamma}}\left)\left]^{N_R}\\
%			\text{high SNR: } P_{out}&=\left(\frac{\gamma_T}{\bar{\gamma}}\right)^{N_R}+O\left(\bar{\gamma}^{-N_R}\left)
%		\end{align*}
		\end{itemize}
	\end{itemize}
