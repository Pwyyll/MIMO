...
\begin{itemize}
	\item we have only the average power constraint: $ \sum\limits_{n = 1}^{N_T} |w_n| = 1 $
	\item SNR:  \scalebox{1.3}[1.3]{$\gamma	 _t = \frac{\epsilon _s|h|^2}{\sigma _n^2} = \frac{\epsilon _s|\sum\limits_{n = 1}^{N_T}w_n^*\cdot h_n |^2}{\sigma _n^2} $} 
	...
	\item constraint optimization problem $\rightarrow $ Lagrange method
		\begin{align*}
			L = \frac{\epsilon _s}{\sigma _n^2}\Bigl | \sum_{n = 1}^{N_T}w_n^*\cdot h_n \Bigr |^2 + \lambda\Bigl ( \sum_{n = 1}^{N_T}|w_n|^2 - 1\Bigr ); \quad \text{where: } \lambda = \text{Lagrange Multiplier}
		\end{align*}
		...
		\begin{itemize}
			...
			\item[$\rightarrow$] \scalebox{1.3}[1.3]{$ \sum\limits_{n = 1}^{N_T} |w_n|^2 = 1 \rightarrow c^2 = \frac{1}{\sum_{n = 1}^{N_T}|h_n|^2}  $}
			\item[$\rightarrow$] \scalebox{1.3}[1.3]{$  w_n = \frac{h_n}{\sqrt{\sum_{n = 1}^{N_T}|h_n|^2}} \equiv $} MRT gains 
		\end{itemize}
		\item[$\rightarrow$] SNR \scalebox{1.3}[1.3]{$ = \frac{\epsilon _s}{\sigma _n^2}\Bigl | \sum\limits_{n = 1}^{N_T}\frac{|h_n|^2}{\sqrt{\sum_{n = 1}^{N_T}|h_m|^2}}\Bigr |^2 = \frac{\epsilon _s}{\sigma _n^2}\sum\limits_{n = 1}^{N_T}|h_n|^2 $}
		\item[$\Rightarrow$] same SNR as for maximum ration combining (MRC)
		\item[$\Rightarrow$] MRT with $N_R t_x $ antennas achieves the same performance as MRC with $N_T r_x $ antennas
		\item[$\Rightarrow$] MRT/MIMO can be extended to $ N_T\times N_R $ MIMO systems
		\begin{itemize}
			\item[$\rightarrow$] has the same performance as MRC with $N_TN_R\; r_x $ antennas and one $t_x $ antenna
		\end{itemize}
	\end{itemize}
...
\begin{itemize}
	...
	\item SNR:
	\begin{align*}
		\gamma _t &= \frac{\epsilon _s}{\sigma _n^2}\Bigl |\sum_{n = 1}^{N_T} w_n^*h_n\Bigr |^2\\
		&= \frac{\epsilon _s}{\sigma _n^2}\Bigl |\sum_{n = 1}^{N_T}\frac{1}{\sqrt{N_T}\cdot \frac{|h_n|^2}{|h_n|}}\Bigr |^2 = \frac{1}{N_T}\cdot \frac{\epsilon _s}{\sigma _n^2}\Bigl |\sum_{n = 1}^{N_T}|h_n|\Bigr |^2\\
		\gamma _n &= \frac{\epsilon _s}{\sigma _n^2}|h_n|^2\\
		&\text{same SNR as for EGC} \rightarrow \gamma _t = \frac{1}{N_T}\Bigl | \sum_{n = 1}{N_T}\sqrt{\gamma _n} \Bigr |^2
	\end{align*}
	\item[$\rightarrow$] EGC with $ N_T\; t_x $  antennas achieves the same performance as EGC with $ N_T\; r_x $ antennas
\end{itemize}
...
0, & \text{otherwise}
\end{cases} \text{where } \hat{n} = \underset{n}{argmax}|h_n|
		\end{align*}
	\item antenna selection with $ N_T\; t_x $ antennas achieves the same performance as \textit{Selection Combining} with $ N_T\;r_x $ antennas
\end{itemize}
\end{document}
