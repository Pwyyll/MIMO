...
\end{align*}
	\item need pdf $\gamma _t$: ($\gamma _n$ bekannt, $\rightarrow$ ges.: Wurzel, etc.)\\ (cumulative distribution function of $\sqrt{\gamma _n}$ (cdf))
	\begin{align*}
	% 1. Formel
	...
	% 5. Formel
		 M_{\sqrt{\gamma _t}}(s) &= \mathcal{E}\Bigl\{exp({-s\sqrt{\gamma _t}})\Bigr\} = \mathcal{E}\Bigl\{exp({-\frac{s}{\sqrt{N_R}}\cdot \sum_{n = 1}^{N_R}\sqrt{\gamma _n}})\Bigr\} = \Bigl(\mathcal{E}\Bigl\{exp(-\frac{s}{\sqrt{N_R}}\cdot \sqrt{\gamma _n}\Bigr\}\Bigr)^{N_R}\\
	% 6. Formel
		 &= \Bigl( M_{\sqrt{\gamma}}\bigl(\frac{s}{\sqrt{N_R}}\bigr) \Bigr)^{N_R} = \Bigl(\frac{2}{\bar{\gamma}}\cdot \frac{N_R}{s^2}\Bigr)^{N_R} + O\Bigl(\frac{1}{\bar{\gamma}\cdot N_R}\Bigr)\\
	% 7. Formel
	...
	% 11. Formel
		P_e &= \frac{a}{\pi}\int_{0}^{\frac{\pi}{2}}M_{\gamma _t}\Bigl(\frac{b}{2\sin^2\theta}\Bigr)~d\theta = \frac{a}{2\pi}\Bigl(\frac{2N_R}{\bar{\gamma}} \Bigr)^{N_R}\cdot\frac{(N_R-1)!}{(2N_R-1)}\cdot\frac{2^{N_R}}{b^{N_R}}\int_{0}^{\frac{\pi}{2}}\sin^{2N_R}\theta \\ \qquad +\; O\bigl(\bar{\gamma}^{-N_R}\bigr) \\
		&= \frac{aN_R^{N_R}}{2b^{N_R}\cdot N_R!}\cdot\frac{1}{\bar{\gamma}^{N_R}} + O\bigl(\bar{\gamma}^{-N_R}\bigr) \overset{!}{=} \Bigl(\frac{1}{G_c\bar{\gamma}}\Bigr)^{G_d}\\
		\text{where: }\sin^{2N_R}\theta~d\theta &= \frac{\pi}{2^{2N_R+1}}\begin{pmatrix}2N_R & N_R	\end{pmatrix} = \frac{\pi(2N_R)!}{2^{2N_R + 1}(N_R!)^2}
	\end{align*}
	\begin{itemize}
